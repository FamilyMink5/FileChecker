...
class CommandCog(commands.Cog):
    ...
    @app_commands.command(name="toggle", description="봇의 기능을 켜거나 끕니다")
    @app_commands.describe(feature="제어할 기능 (save_temp, file_check, link_check)")
    async def toggle_feature(self, interaction: discord.Interaction, feature: str):
        ...
        if feature not in ["save_temp", "file_check", "link_check"]:
            await interaction.response.send_message("❌ 잘못된 기능 이름입니다. 사용 가능한 기능: save_temp, file_check, link_check", ephemeral=True)
            return
        ...
        await interaction.response.send_message(f"✅ {feature_names[feature]}이(가) {status}되었습니다.")
        logging.info(f"Feature {feature} toggled to {new_value} for server {guild_id}")

    @app_commands.command(name="clear", description="Temp 폴더를 초기화합니다")
    async def clear_temp(self, interaction: discord.Interaction):
        if not is_admin(interaction):
            await interaction.response.send_message("이 명령어는 관리자만 사용할 수 있습니다.", ephemeral=True)
            return
        ...
        try:
            ...
            await interaction.response.send_message("✅ Temp 폴더가 초기화되었습니다.")
    ...
    @app_commands.command(name="set", description="봇 설정을 변경합니다")
    @app_commands.describe(network="네트워크 대역폭 제한 (Mbps, 0은 무제한)")
    async def set_network(self, interaction: discord.Interaction, network: int):
        if not is_admin(interaction):
            await interaction.response.send_message("이 명령어는 관리자만 사용할 수 있습니다.", ephemeral=True)
            return
        ...
        await interaction.response.send_message(f"✅ 네트워크 대역폭이 {status_msg}으로 설정되었습니다.")
